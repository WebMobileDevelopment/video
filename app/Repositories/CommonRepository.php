        ...
        try {
            ...
            if($request->hasfile('video'))
                $validator = Validator::make( $request->all(), array(
                    ...
                    'video'     => 'mimes:mp4,vob,wmv,avi,mov,asf,av1,vp9,r4d',
            ...
            if( $validator->fails() ) {
                ...
                if (in_array($request->video_type , [ VIDEO_TYPE_YOUTUBE , VIDEO_TYPE_OTHERS] )) {
                    ...
                    $uploadVideovalidator = Validator::make( $request->all(), array(
                        'video'=>$request->id ? 'mimes:mp4,vob,wmv,avi,mov,asf,av1, vp9, r4d' : 'required|mimes:mp4,vob,wmv,avi,mov,asf,av1, vp9, r4d',
                    ));
                ...
                if ($model->id) {
                    ...
                    if ($request->video_type == VIDEO_TYPE_UPLOAD) {
                        ...
                        $uploadVideovalidator = Validator::make( $request->all(), array(
                            'video'=>'required|mimes:mp4,vob,wmv,avi,mov,asf',
                        ));
    ...
    public static function r4d_video_save(Request $request) {
        ...
        try {
            ...
            $validator = Validator::make( $request->all(), array(
                        ...
                        'file'=>'required|mimes:mp4,vob,wmv,avi,mov,asf'
                    ));
            ...
            if( $validator->fails() ) {
                ...
                // $model = $request->has('id') ? VideoTape::find($request->id) : new VideoTape;
                $model = VideoTape::where('title', $request->title)->where('video_type', VIDEO_TYPE_R4D)->first();
                ...
                if($model == null) {
                    ...
                    if($request->file('file')->getMimeType()) {

                        if($request->hasfile('file'))
                            $uploadVideovalidator = Validator::make( $request->all(), array(
                                'file'=>$request->id ? 'mimes:mp4,vob,wmv,avi,mov,asf' : 'required|mimes:mp4,vob,wmv,avi,mov,asf'
                            ));
            
                        if($uploadVideovalidator->fails()) {
                            $error_messages = implode(',', $uploadVideovalidator->messages()->all());
                            throw new Exception('The file must be a file of type: mp4, vob, wmv, avi, mov.');
                        }
                    }
                    }
                    ...
                    if(in_array($request->video_type , [ VIDEO_TYPE_YOUTUBE , VIDEO_TYPE_OTHERS] )) {
                        ...
                        if($request->hasfile('file')) {
                            ...
                            if(isset($request->uploadfolder) && $request->video_type == VIDEO_TYPE_R4D) {
                                ...
                                $uploadfolder = '/uploads/videos/'.$user_name.'/'.$title.'/'.$request->uploadfolder.'/'; //.'_'.time()
                            ...
                            if($request->hasfile('file')) {
                                ...
                                $org_n = pathinfo($original_name, PATHINFO_FILENAME );
                                $main_video_duration = Helper::video_upload($request->file('file'), $org_n, $uploadfolder);
                            }
                            ...
                            if($request->has('file')) {
                                if ($request->file('file')->getClientSize()) {
            
                                    $bytes = convertMegaBytes($request->file('file')->getClientSize());
   ...
   public static function webIosUrl($model) {
    ...
    public static function r4d_upload_video_image($request) {
        ...
        if ($temp_video)  {
            ...
            if(($org_video_title != $new_video_title) && File::isDirectory('uploads/videos/'.$user_name.'/'.$org_video_title)) {
                File::copyDirectory(public_path('uploads/videos/'.$user_name.'/'.$org_video_title), 
                    public_path('uploads/videos/'.$user_name.'/'.$new_video_title));

                $temp_folder_list = File::directories(public_path('uploads/videos/'.$user_name.'/'.$org_video_title));
                // dd($temp_folder_list);
                ...
                File::deleteDirectory(public_path('uploads/videos/'.$user_name.'/'.$org_video_title));
            }
            
            try {
                $video = VideoTape::find($temp_video->id);
                if ($video) {
                    $required = 0;
                    ...
                    if ($default_image && $request->hasfile('r4d_cover_img'))  {
                        ...
                        $model = VideoTape::find($temp_video->id);
